  ...
  class Config
    describe CNetwork do
      ...
      describe "#image_url" do
        let(:network_attr) { :image_url }
        it_behaves_like "a config network attribute"

        it "expect :image alias" do
          expect(c_network.available_attributes).to include(:image)

          # network_attr_val = "Title val"
          # expect(c_network).to receive(network_attr).with(network_attr_val)
          # c_network.image network_attr_val

          # expect(c_network.method(:image)).to be == c_network.method(:image_url)
        end
